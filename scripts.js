...
$( document ).ready(function() {
    listLanguages();
    getLatestRepos(per_page, date, lang);
    $('#date').val('2019-01-10');
});

// Get latest repositories based on the selected criteria
function getLatestRepos(per_page, date, lang) {
    $.ajax({
        type: "POST",
        url: "./GitReposController.php",
        dataType: "json",
        data:{"per_page":per_page, "date":date, "lang":lang}
...
// Set the perpage var by the selected value.
function perPage(){
    per_page = $('#mySelect').val();
    getLatestRepos(per_page, date, lang); 
}

// Set the perpage var by the selected language.
function perLang(language){
    lang = language;
    getLatestRepos(per_page, date, lang);
    document.getElementById("myDropdown").classList.toggle("show");
}

// Set the perpage var by the selected startDate.
function startDate(){
    date = $('#date').val();
    getLatestRepos(per_page, date, lang); 
}
...
function listLanguages(){
    ...
    document.getElementById("lang").innerHTML = '<option id="0" disable> Choose Language</option>';
    languages.forEach((element, i) => {
        element = element.replace(/\s/g, '');
        var opt = document.createElement("option");
        document.getElementById("lang").innerHTML += '<option style="cursor:pointer" onclick=perLang("' + element.toLowerCase() + '") value="' + element + '" id="' + i + '">' + element + '</option>';
    });
}

// Show/hide Choose Language button
function toogleLangButton() {
    document.getElementById("myDropdown").classList.toggle("show");
}
 
// Search on language list
function filterLanguages() {
    var input, filter, i, option;
